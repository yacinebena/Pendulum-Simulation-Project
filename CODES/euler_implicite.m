function [temps, solution] = euler_implicite(f, intervalle_temps, y0, pas, tol)
    ...
    for i = 1:(n_steps-1)
        ...
        % Définir une estimation initiale pour y_{n+1}
        y_next = y_n;

        % Résoudre l'équation implicite par méthode de point fixe
        diff = inf;
        while diff > tol
            y_next_old = y_next;
            y_next = y_n + pas * f(t_n + pas, y_next);
